...
const Test = () => {
    const initWallet = async () => {
        ...
        const result = axios.post(ENDPOINTS.WALLET.INIT, data, config);
        console.log(result);
    };

    const getAddresses = async () => {
        const config = {headers: {api_key: API_KEY}};
        const result = await axios.get(ENDPOINTS.WALLET.ADDRESSES, config);
        console.log(result);
    };

    const lockWallet = async () => {
        const config = {headers: {api_key: API_KEY}};
        const result = await axios.get(ENDPOINTS.WALLET.LOCK, config);
        console.log(result);
    };
    ...
    const unlockWallet = async () => {
        ...
        const config = {headers: {'Content-Type': 'application/json;charset=UTF-8', api_key: API_KEY}}
        const result = await axios.post(ENDPOINTS.WALLET.UNLOCK, data, config);
        console.log(result);
    };
