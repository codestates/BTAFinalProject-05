...
import BlockchainUtil from '../utility/BlockchainUtil';
...
export default class WalletService {
  static async generateMnemonicCode() {
    try {
      let mnemomic = keystore.generateRandomSeed();
      return mnemomic;
    } catch (err) {
      throw(err);
    }
  ...
  static async transfer(fromAddress: string, toAddress: string, amount: string, password: string, res: any) {
    try {
      ...
      keyStoreInstance.keyFromPassword(password, async (err: any, pwDerivedKey: any) => {
        ...
        const gasInfo = await BlockchainUtil.getCurrentGasPrices();
        ...
        await BlockchainUtil.sendSignedTransaction(params, privateKey, web3, res);
      });
    } catch (err) {
      throw(err);
    }
